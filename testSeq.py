...
parser.add_argument("-t", "--test", help="Check files for sequence name collitions", action="store_true")
parser.add_argument("files", nargs="*", type=str, help="Input files to search for seq. name collitions")
...
def test():
	seqIdDict = {}
	# Count the number of sequences in the files.
	numSeq = 0.0
	# The infile order number will be stored in the dictionary.
	inFileNr = 0
	for inFile in args.files:
		inFileNr += 1
		f = fastqFile(inFile, 'r')
		
		# Identify the delimiter after y-coord in sequence ids.
		for firstSeq in f.getSeqs():
			if '#' in firstSeq.getId():
				delim = '#'
			else:
				delim = ' '
			break
				
		# Place sequence id's in a dictionary.
		f = fastqFile(inFile, 'r')
		for seq in f.getSeqs():
			numSeq += 1
			# Create an entry if id is not already in dictionary.
			if seqIdDict.has_key(seq.getId().split(delim)[0]):
				seqIdDict[seq.getId().split(delim)[0]].append(inFileNr)
			else:
				seqIdDict[seq.getId().split(delim)[0]] = [inFileNr]
		
		print "%s: %s" % (inFileNr, inFile)
	
	# Make some noise if triplets are found.
	if numSeq/2 > len(seqIdDict):
		print "[WW] Only found %s paired sequences among %s sequences." % (len(seqIdDict), int(numSeq))
	else:
		print "Found %s unique sequence id's" %  len(seqIdDict)

		
	for key, value in seqIdDict.iteritems():
		if len(value) >= 3:
			print key, value
